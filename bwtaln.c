...
void bwa_cal_sa_reg_gap(int tid, bwt_t *const bwt, int n_seqs, bwa_seq_t *seqs, const gap_opt_t *opt)
{
	...
	for (i = 0; i != n_seqs; ++i) {
		bwa_seq_t *p = seqs + i;
#ifdef HAVE_PTHREAD
		if (i % opt->n_threads != tid) continue;//划分线程数据
#endif
		...
		p->aln = bwt_match_gap(bwt, p->len, p->seq, w, p->len <= opt->seed_len? 0 : seed_w, &local_opt, &p->n_aln, stack);//后缀数组区间的计算。
...
//prefix:reference sequence, fn_fa:reads, opt:
void bwa_aln_core(const char *prefix, const char *fn_fa, const gap_opt_t *opt)
{
	...
	double restore_bwt_time = 0;//
	double sai_store_time = 0;//
	double read_fa_time = 0;
	double destroy_time = 0;
	double bwa_cal_sa_reg_gap_time = 0;
	double aln_time = realtime();

	int times = 0;
	// initialization
	t_real1 = realtime();
	ks = bwa_open_reads(opt->mode, fn_fa);   //reads序列
	read_fa_time = realtime() - t_real1;
	{ // load BWT
		t_real4 = realtime();
		char *str = (char*)calloc(strlen(prefix) + 10, 1);
		strcpy(str, prefix); strcat(str, ".bwt");  bwt = bwt_restore_bwt(str);
		free(str);
		restore_bwt_time = realtime() - t_real4;//load_bwt
	}
	// core loop
	t_real5 = realtime();
	err_fwrite(SAI_MAGIC, 1, 4, stdout);//.sai中间文件写入验证信息
	err_fwrite(opt, sizeof(gap_opt_t), 1, stdout);
	sai_store_time = realtime() - t_real5;
	t_real1 = realtime();
	while ((seqs = bwa_read_seq(ks, 0x40000, &n_seqs, opt->mode, opt->trim_qual)) != 0) {
		...
		if (opt->n_threads <= 1) { // no multi-threading at all
			t_real5 = realtime();
			bwa_cal_sa_reg_gap(0, bwt, n_seqs, seqs, opt);//计算SA位置
			bwa_cal_sa_reg_gap_time = bwa_cal_sa_reg_gap_time + realtime() - t_real5;
		} else {
			pthread_t *tid;
			pthread_attr_t attr;
			thread_aux_t *data;
		...
		sai_store_time = sai_store_time + realtime() - t_real3 ; //保存sam文件时间
		//fprintf(stderr, "%.2f sec\n", (float)(clock() - t) / CLOCKS_PER_SEC);

		bwa_free_read_seq(n_seqs, seqs);
		//fprintf(stderr, "[bwa_aln_core] %lld sequences have been processed.\n", tot_seqs);
		time_intv2  = time_intv2 + realtime() - t_real2;//循环内部总时间
	}
	time_intv1 = realtime() - t_real1;//while循环总时间
	read_seq_time = time_intv1 - time_intv2;//read_seq时间
	// destroy
	t_real1 = realtime();
	bwt_destroy(bwt);
	bwa_seq_close(ks);
	destroy_time = realtime() - t_real1;
	aln_time = realtime() - aln_time;
	//aln_time = read_fa + restore_bwt + read_seq + bwa_cal_sa_reg_gap_time + sai_store + destroy_time
	freopen("../bwa33.txt","a",stderr);
	fprintf(stderr,"occ_intv:%d,read_name:%s,aln_time:%.3f = read_fa:%.3f + restore_bwt:%.3f + read_seq:%.3f + bwa_cal_sa_reg_gap_time:%.3f + sai_store:%.3f + destroy:%.3f\n",OCC_INTERVAL,fn_fa,aln_time,read_fa_time,restore_bwt_time,read_seq_time,bwa_cal_sa_reg_gap_time,sai_store_time,destroy_time);
	fclose(stderr);

}
	int OCC_INTV_SHIFT = 7;
	int OCC_INTERVAL;//OCC_INTERVAL=2^OCC_INTV_SHIFT
	int OCC_INTV_MASK;
